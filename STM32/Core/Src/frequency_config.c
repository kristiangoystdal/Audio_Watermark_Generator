...
freq_pair_t find_frequency_pair(uint32_t fs) {
    ...
    const size_t min_bit_samples =
        (size_t)((((uint64_t)fs * (uint64_t)MIN_BIT_US) + 500000ull) / 1000000ull);
    ...
    const uint16_t lower_samples_per_period =
        (uint16_t)floor((double)fs / (double)lower_freq);
    const uint16_t higher_samples_per_period =
        (uint16_t)floor((double)fs / (double)higher_freq);

    // Number of periods needed to approximately satisfy minimum bit duration
    const uint16_t lower_period_count =
        (uint16_t)llround((double)min_bit_samples / (double)lower_samples_per_period);
    const uint16_t higher_period_count =
        (uint16_t)llround((double)min_bit_samples / (double)higher_samples_per_period);
    ...
    LOGF("Lower freq samples: %u, periods: %u, total samples: %u\r\n",
         (unsigned int)lower_samples_per_period,
    ...
    LOGF("Higher freq samples: %u, periods: %u, total samples: %u\r\n",
         ...
         (unsigned int)higher_period_count,
         (unsigned int)higher_total_samples);
