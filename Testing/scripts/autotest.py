...
def _add_noise(
    audio: np.ndarray, fs: float, f0: float, f1: float, snr_db: float
) -> np.ndarray:
...
def _decode_file_worker(
    src: str, f0: float, f1: float, snr_db, result_queue: mp.Queue
) -> None:
...
def run_subfolder(
    folder_path: str, f0: float, f1: float, snr_db
) -> tuple[int, float, str]:
...
def main() -> None:
    ...
    for snr_db in SNR_LEVELS_DB:
        ...
        for folder in subfolders:
            ...
            note = err or (
                "" if status == "PASS" else f"got {total}, expected {EXPECTED_MESSAGES}"
            )
            print(
