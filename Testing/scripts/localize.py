...
_RESULTS_DIR = os.path.normpath(
    os.path.join(_SCRIPT_DIR, "../results/acoustic_localization")
)
...
_DATASETS = {
    "1": {
        "data_dir": os.path.normpath(
            os.path.join(
                _SCRIPT_DIR, "../test_files/acoustic_localization/dødens_dal_1"
            )
        ),
    ...
    "2": {
        "data_dir": os.path.normpath(
            os.path.join(
                _SCRIPT_DIR, "../test_files/acoustic_localization/dødens_dal_2"
            )
        ),
    ...
    "3": {
        "data_dir": os.path.normpath(
            os.path.join(_SCRIPT_DIR, "../test_files/acoustic_localization/echo_room")
        ),
...
LOCALIZATION_ALGORITHM = (
    "least_squares"  # "gillette", "soundfinder", or "least_squares"
)
...
if FORCE_REPROCESS or not (
    os.path.exists(_ARU_COORDS_CSV) and os.path.exists(_DETECTIONS_CSV)
):
    ...
    for filename in MIC_GPS:
        ...
        print(
            f"  {filename}: {len(recording.detections)} detections above {MIN_CONFIDENCE:.0%}"
        )

    all_species = sorted(
        {d["common_name"] for dets in raw_detections.values() for d in dets}
    )
...
if USE_TEMPERATURE_SPEED_OF_SOUND:
    _mic2_txt = os.path.join(
        AUDIO_DIR, os.path.splitext(_cfg["mic_filenames"][1])[0] + ".txt"
    )
    _avg_temp_c = _avg_temp_from_txt(_mic2_txt)
    SPEED_OF_SOUND = 331.3 + 0.606 * _avg_temp_c
    print(
        f"Average temperature (ARU 2): {_avg_temp_c:.1f}°C → speed of sound: {SPEED_OF_SOUND:.1f} m/s"
    )
else:
...
position_estimates = [
    ...
    if np.hypot(
        e.location_estimate[0] - array_centre[0],
        e.location_estimate[1] - array_centre[1],
    )
    <= MAX_ARRAY_DIST
]

print(
    f"Total position estimates (residual ≤ {RESIDUAL_THRESHOLD} m, within {MAX_ARRAY_DIST} m of array centre): {len(position_estimates)}"
)
...
if species_list:
    ...
    for sp in sorted(species_list):
        ...
        if best_file is not None:
            print(
                f"  {sp}: {best_file}  t={best_t:.0f}s ({best_ts.strftime('%H:%M:%S')})  conf={best_conf:.2f}"
            )
...
ax.scatter(
    aru_pix[:, 0], aru_pix[:, 1], marker="^", color="red", s=60, zorder=5, label="ARU"
)
...
for sp, (best_est, best_conf, best_ts) in best_estimates.items():
    ...
    pix_sp = utm_to_pixel(
        [best_est.location_estimate[0]], [best_est.location_estimate[1]]
    )
    ax_sp.scatter(
        pix_sp[0, 0],
        pix_sp[0, 1],
